...
public class UpdateUtil {
	...
	public JSONObject chechUpdate() {
		...
		try {
			...
			if (NetworkUtil.dataConnected(context)) {
				...
				new UpdateTask(YockerApplication.appVersion).execute();
				//} else {
	...
	private class UpdateTask extends AsyncTask<Integer, Integer, JSONObject> {
		...
		protected JSONObject doInBackground(Integer... params) {
			...
			try {
				String urlPath = PathConsts.URL_UPDATE;
				Log.e("地址：", urlPath+"");
				updateInfo = NetworkUtil.getJsonObject(urlPath, null, CommonConsts.NETWORK_TIMEOUT_LENGTH);
