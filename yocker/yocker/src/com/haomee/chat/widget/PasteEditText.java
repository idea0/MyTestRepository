...
public class PasteEditText extends EditText {
	...
	public boolean onTextContextMenuItem(int id) {
		if (id == android.R.id.paste) {
			...
			if (text.startsWith(MediaPlayerActivity.COPY_IMAGE)) {
				// intent.setDataAndType(Uri.fromFile(new
				// File("/sdcard/mn1.jpg")), "image/*");
				text = text.replace(MediaPlayerActivity.COPY_IMAGE, "");
				Intent intent = new Intent(context, AlertDialog.class);
				intent.putExtra("title", "发送以下图片？");
				intent.putExtra("forwardImage", text);
				intent.putExtra("cancel", true);
				((Activity) context).startActivityForResult(intent, MediaPlayerActivity.REQUEST_CODE_COPY_AND_PASTE);
				// clip.setText("");
			}
		}
	...
	protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
		if (!TextUtils.isEmpty(text) && text.toString().startsWith(MediaPlayerActivity.COPY_IMAGE)) {
			setText("");
		}
